...
func _ready():
	...
	PlayerManager.on_joystick_touch.connect(func(vector: Vector2, strength: float) -> void:
		_joystick_input = vector
	)
	PlayerManager.on_joystick_touch_end.connect(func() -> void:
		_joystick_input = Vector2.ZERO
	)
	LevelManager.level_load_started.connect(func() -> void:
		_joystick_input = Vector2.ZERO
	)
	pass
