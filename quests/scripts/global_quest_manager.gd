...
func update_quest( _title : String, _completed_step : String = "", _is_complete : bool = false ) -> void:
	...
	if quest_index == -1:
		...
		PlayerHud.queue_notification( "Задание началось", _title )
		pass
	else:
		...
		if q.is_complete == true:
			PlayerHud.queue_notification( "Задание завершено!", _title )
			disperse_quest_rewards( find_quest_by_title( _title ) )
		else:
			PlayerHud.queue_notification( "Задание обновилось", _title + ": " + _completed_step )
	pass
...
func disperse_quest_rewards( _q : Quest ) -> void:
	...
	var _message : String = str( _q.reward_xp ) + " опыта"
	...
	PlayerHud.queue_notification( "Награды получены!", _message )
	
	if is_all_quests_completed():
		PlayerHud.queue_notification( "Поздравляем!", 'Ты прошел все задания. \nЗайди в первый корпус для приза' )
...
func find_quest( _quest : Quest ) -> Dictionary:
	for q in current_quests:
		if q.title.to_lower() == _quest.title.to_lower():
			return q
	return { title = "не найдено", is_complete = false, completed_steps = [''] }
...
func is_all_quests_completed() -> bool:
	var completed_count: int = 0
	for quest_dict: Dictionary in QuestManager.current_quests:
		if quest_dict.get('is_complete', false):
			completed_count += 1
	
	if (completed_count == QuestManager.quests.size()):
		return true
	return false
