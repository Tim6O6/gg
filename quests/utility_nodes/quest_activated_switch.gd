...
@export var remove_when_completed : bool = true
...
func check_is_activated() -> void:
	...
	var do_show_when_activated := (not remove_when_activated)
	if remove_when_completed and (_q.get('is_complete', false) == true):
		set_is_activated( remove_when_activated )
		return
	
	if _q.title != "не найдено":
