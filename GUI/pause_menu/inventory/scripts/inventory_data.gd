...
func set_current_item( slot : SlotData ) -> void:
	if slot == null:
		current_item_index = -1
		return
	
	var slot_index : int = slots.find( slot )
	current_item_index = slot_index
...
func equip_item( slot : SlotData ) -> void:
	...
	if not (slot.item_data is EquipableItemData):
		return
...
func get_slot(slot_index: int) -> SlotData:
	if (slot_index == -1) or (slot_index > (slots.size() - 1)):
		return
	if slots[slot_index] != null and slots[slot_index].quantity <= 0:
		return
	return slots[slot_index]

func get_current_slot() -> SlotData:
	return get_slot(current_item_index)

func get_slots_with_item( _item: ItemData ) -> Array[SlotData]:
	var slots_array: Array[SlotData] = []
	for slot in slots:
		if slot:
			if slot.item_data:
