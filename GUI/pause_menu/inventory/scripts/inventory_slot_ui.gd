...
func set_current_item() -> void:
	PlayerManager.INVENTORY_DATA.set_current_item( slot_data )


func item_pressed( from_keyboard := false ) -> void:
	if slot_data and (from_keyboard or outside_drag_threshold() == false):
