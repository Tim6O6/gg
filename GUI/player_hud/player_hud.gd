...
signal on_quest_tab_visibility_changed(state: bool)
signal on_backpack_tab_visibility_changed(state: bool)
...
@onready var mobile: Control = $Control/mobile
...
func _ready():
	...
	if not OS.has_feature('mobile') and not OS.is_debug_build():
		mobile.queue_free()
